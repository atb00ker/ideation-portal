...
const Home = () => {
  ...
  const {
    ...
    data: topicsData,
    error: topicsError,
  } = GetTopicsCollection(pageSize, pageSize * pageActive);
      ...
      <Col xs={12}>
        <Row className='m-auto max-width-960'>
          <Col className='ps-0' sm={10} md={11}>
            Do you have an Idea to share? A challenge your team is facing? We want to hear about it! Please
            click the 'add' button to share.
          </Col>
  ...
  if (countLoading || topicsLoading) return <SectionLoader height='500px' width='100%' />;
      ...
      <Row className='max-width-960 mx-auto'>
        <Col xs={12} className='mt-2 mb-5 p-0'>
          <ErrorBoundary FallbackComponent={ErrorFallback}>
            ...
            {pagesCount > 1 && (
              <Pagination className='mb-4 mt-4 float-end' size='sm'>
                <Pagination.Prev />
